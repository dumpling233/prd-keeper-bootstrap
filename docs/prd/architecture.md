...
`prd-keeper-bootstrap` 是一个以文件模板和 Python 标准库脚本为核心的离线 CLI 工具。仓库本身不包含常驻服务、前端应用或数据库。

- 前端：无前端界面。
- 后端：无后端服务；核心逻辑是本地命令行脚本 `scripts/init_prd_keeper.py`。
- 数据层：无数据库；所有输出均为目标项目中的仓库文件。
- 外部服务：初始化过程不依赖外部服务；脚本仅可选读取目标项目的 Git 配置。
- 部署方式：作为 Git 仓库或本地目录使用；运行时通过 Python 执行初始化脚本。
...
1. 用户指定目标目录和项目名称。
2. `scripts/init_prd_keeper.py` 读取目标项目 Git 配置，计算作者名和安全文件名片段。
3. 脚本递归读取 `templates/` 下的模板文件。
4. 脚本渲染模板变量，并将 `.tpl` 文件写入目标项目对应路径。
5. 如果目标文件已存在且未传入 `--force`，脚本跳过该文件。
6. 脚本创建或追加目标项目 `docs/prd/prd_log/YYYYMMDD-{git-user}.md` 初始化日志。
7. 用户在目标项目中运行生成的 `check_prd_keeper.py` 校验治理结构。

## 权限、配置、环境变量和第三方服务边界

- 权限/鉴权：不涉及账号、登录、角色或鉴权流程。
- 配置项：当前通过命令行参数 `--target`、`--project-name` 和 `--force` 控制。
- 环境变量：脚本自身没有必需环境变量；实际运行依赖系统能找到可执行的 Python。
- 第三方服务：无必需第三方服务；Git 仅用于读取本地 `user.name` 或 `user.email`。
